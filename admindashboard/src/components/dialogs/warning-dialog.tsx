...
export const WarningDialog = ({ content, open, handleClick }: {
  ...
  return (
    <Dialog onClose={() => { handleClick(WarningOption.No); }} open={open} >
      <Stack p={4.4} gap={0.5} sx={{
        ...
        <Stack direction='row' gap={3}>
          <StyleWarningButton
            onClick={() => { handleClick(WarningOption.Yes); }}>
            Yes
          </StyleWarningButton>

          <StyledButton
            onClick={() => { handleClick(WarningOption.No); }}
            variant="outlined"
            sx={{
              color: white,
