...
import type { AllModels, HandleTableTypes, HeadLabelParameters, TableOptions } from "@interface";
import { AllModelsEnum, LaboratoryTableLabel, TableOrder, UserTableLabel } from "@interface";
...
export function HandleTable(
  { data, initialOrder, modelType, options }: {
    data: AllModels[];
    initialOrder?: string;
  ...
  const [orderBy, setOrderBy] = useState<string>(initialOrder ?? headLabel[0].id);
  ...
  const displayPagination = options?.hidePagination ?? dataCount <= rowsPerPage ? 'none' : undefined;
